...
実装のたびに以下の観点でも確認する：
- 操作が直感的か（迷わずタップ・クリックできるか）
- モバイル幅（375px）で文字・ボタンが窮屈でないか
- 最頻ユースケース（今日の空き検索・エリア絞り込み）が最小タップ数で達成できるか
- フィルタ状態が視覚的に明確か（ON/OFF が一目でわかるか）
- 不必要な情報で画面が埋まっていないか（Progressive Disclosure を意識）
