...
export interface NormalizedVenue {
  ...
  rooms: { name: string; price_per_hour: number | null; capacity: number | null; open_hour: number | null; close_hour: number | null; facilities: string[]; image_url: string | null; source_url: string | null }[]
}
...
export abstract class BaseScraper {
  ...
  normalize(rawVenues: RawVenue[]): NormalizedVenue[] {
    return rawVenues.map((v) => ({
      name: v.name, area: "大阪市", city: v.city ?? null, nearest_station: v.nearestStation ?? null,
      address: v.addressRaw ?? null, lat: null, lng: null, description: v.description ?? null,
      image_url: v.imageUrl ?? null, source_site: this.siteName, source_url: v.sourceUrl ?? null,
      rooms: v.rooms.map((r) => ({
        name: r.name, price_per_hour: parsePrice(r.priceRaw), capacity: parseCapacity(r.capacityRaw),
        open_hour: parseOpenHour(r.openHourRaw), close_hour: parseCloseHour(r.closeHourRaw),
        facilities: r.facilities ?? [], image_url: r.imageUrl ?? null, source_url: r.sourceUrl ?? null,
      })),
    }))
  }
